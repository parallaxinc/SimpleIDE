...
void ReplaceDialog::findPrevClicked()
{
    ...
    if (edtext.contains(text,Qt::CaseInsensitive)) {
        editor->setCenterOnScroll(true);
        if(editor->find(text,QTextDocument::FindBackward) == true) {
            editor->activateWindow();
        }
        else {
            QTextCursor cur = editor->textCursor();
...
void ReplaceDialog::replacePrevClicked()
{
    ...
    if (edtext.contains(text,Qt::CaseInsensitive)) {
        editor->setCenterOnScroll(true);
        if(editor->find(text,QTextDocument::FindBackward) == false) {
            QTextCursor cur = editor->textCursor();
            cur.movePosition(QTextCursor::End, QTextCursor::MoveAnchor);
            editor->setTextCursor(cur);
            replacePrevClicked();
...
bool ReplaceDialog::replaceBeginMessage()
{
    QMessageBox::StandardButton ret = QMessageBox::information(
        this, tr("Replace ?"),tr("Replace from beginning?"),
